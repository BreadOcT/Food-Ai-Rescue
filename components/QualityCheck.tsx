...
import { Camera, Sparkles, CheckCircle, AlertCircle, ShieldCheck as ShieldIcon, Zap, Loader2, Info, Clock, ThermometerSnowflake, ShieldCheck } from 'lucide-react';
...
/**
 * AnalysisDashboard: Renders the detailed results from Gemini AI analysis.
 */
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom duration-500">
      ...
      <Section title="Panduan & Dampak">
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
             <Card className="p-4 bg-emerald-50 dark:bg-emerald-900/10 border-emerald-100 dark:border-emerald-900/20 text-center">
                <p className="text-[10px] font-black text-emerald-600 uppercase mb-1 tracking-widest">CO2 Saved</p>
                <p className="text-sm font-black text-emerald-700 dark:text-emerald-400">{result.environmentalImpact.co2Saved}</p>
             </Card>
             <Card className="p-4 bg-blue-50 dark:bg-blue-900/10 border-blue-100 dark:border-blue-900/20 text-center">
                <p className="text-[10px] font-black text-blue-600 uppercase mb-1 tracking-widest">Water Saved</p>
                <p className="text-sm font-black text-blue-700 dark:text-blue-400">{result.environmentalImpact.waterSaved}</p>
             </Card>
          </div>
        </div>
      </Section>
      ...
      <div className="flex flex-col gap-3 pt-6 pb-20">
        <Button className="w-full h-14 rounded-2xl shadow-xl shadow-primary/20 font-black uppercase tracking-[0.2em] text-[10px]" onClick={onGoToRecipes}>Cari Resep dari Bahan Ini</Button>
...
/**
 * QualityCheckScreen: Main entry point for the AI Quality Check feature.
 */
export const QualityCheckScreen: React.FC<QualityCheckProps> = ({ goBack, globalState, setGlobalState }) => {
  ...
  const handleAnalysis = async (base64: string) => {
    ...
    setIsAnalyzing(true);
    try {
      // Step 1: Request AI analysis for quality verification
      const analysis = await analyzeFoodQuality(["Hasil scan kamera"], base64);
      setResult(analysis);
      
      // Step 2: Store analysis results in global history
      ...
      <ScrollableContent className="p-6">
        {isAnalyzing ? (
          ...
          <AnalysisDashboard 
            ...
            onGoToRecipes={() => alert("Fitur Integrasi Resep Cookpad segera hadir!")}
          />
        ) : (
          <div className="space-y-12 py-6">
            ...
            >
              ...
              <div className="text-center">
                ...
                <p className="text-[10px] text-slate-400 font-bold mt-1 uppercase">Scan atau Unggah Foto</p>
              </div>
              <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={onFileChange} />
            </div>

            <Section title="Manfaat Analisis">
               <div className="grid grid-cols-1 gap-4">
                  {[
                    { icon: <Zap size={20} />, title: "Skor Higienitas", desc: "Verifikasi standar kebersihan secara visual.", color: "bg-yellow-100 text-yellow-600" },
                    { icon: <ShieldIcon size={20} />, title: "Halal Detector", desc: "Mendeteksi item non-halal pada komposisi.", color: "bg-emerald-100 text-emerald-600" },
                    { icon: <Clock size={20} />, title: "Masa Simpan", desc: "Prediksi durasi kelayakan konsumsi aman.", color: "bg-blue-100 text-blue-600" }
                  ].map((info, i) => (
                    <Card key={i} className="p-5 flex items-center gap-5 bg-white dark:bg-slate-900 border-slate-100 dark:border-slate-800 shadow-sm">
                      <div className={`w-12 h-12 rounded-2xl ${info.color} flex items-center justify-center shrink-0`}>
                        {info.icon}
                      </div>
                      <div>
                        <h4 className="font-black text-sm text-slate-900 dark:text-white uppercase tracking-tight">{info.title}</h4>
                        <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium leading-tight mt-0.5">{info.desc}</p>
                      </div>
                    </Card>
                  ))}
               </div>
